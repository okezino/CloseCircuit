...
class RegisterUseCaseTest {
    ...
    fun `flow emit loading result first`() = runBlocking{
        val registerRequest = RegisterRequest(email, name, roles, phone_number,password, confirm_password)
    ...
    fun `flow emit success when registered with correct credentials`() = runBlocking{
        ...
        val registerResult = Result(
            ...
            RegisterResponseDto(email, name, roles, phone_number),
    ...
    fun `Register with Incorrect Details and Return Error`() = runBlocking {
        ...
        val registerRequest = RegisterRequest(email, name, roles, "4",  "", confirm_password)
