...
class ChangePasswordUseCase@Inject constructor(private val settingsRepositoryInterface: SettingsRepositoryInterface) {
    suspend operator fun invoke(changePasswordRequest: ChangePasswordRequest, userId: String): Flow<ChangePasswordResponseType> =
        settingsRepositoryInterface.changePassword(changePasswordRequest, userId)
}